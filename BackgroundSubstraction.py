...
while True:
    ...
    # grayscale_img = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    # norm_image = cv2.normalize(grayscale_img, norm_image, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)

    # frame = helpers.normalize_image(grayscale_img)
